...
### Trigger a chatbot command on a click of a button

1. Head over to the `Settings` tab of your button and add a dedicated `ID`
2. In your chatbot `Embed` element, insert this code in the existing `<script>` tag:
...
```html
<script type="module">
  import Chatbot from 'https://cdn.jsdelivr.net/gh/buildchat-bot/embeds@v1.1/js/web.js'
