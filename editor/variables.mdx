...
Variables are placeholders for content that you can then use anywhere in the Chatbot. It's a very important concept to understand to truly create a customized experience for the user.

## Save an answer in a variable

You can tell your input block to save the answer into a variable and reuse then in a further bubble.
...
Let's say my chatbot contains these variables:
...
`https://buildchat.bot/my-bot?Email=test@test.com&First%20name=John` (Note that spaces in variable names should be replaced by `%20`)
